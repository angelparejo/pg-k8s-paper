...
### 2026-08-17 — writer-critic / verifier (contraste con el documento revisado por la tutora)
**Phase:** Peer Review (revisión de supervisión académica)
**Target:** paper/faraute/main.tex frente a articulo_angelparejov2-experimental_entregable-bea.docx
**Score:** N/A (contraste de fidelidad, no puntuación)
**Verdict:** FIDELIDAD CONCEPTUAL ÍNTEGRA. Mismas 8 secciones y orden, tesis multicapa, modelo S=(O,K,M,D) con su justificación de notación, 4 dimensiones, 3 invariantes, Tabla I, metodología completa y las 34 cifras comprobadas sin una sola alteración; referencias 27↔27 (mismo corpus salvo +Kingsbury & Alvaro y +doc PostgreSQL). El cuerpo bajó de ~8.581 a ~4.270 palabras (−50 %): se recortó desarrollo argumental y estadística fina (al suplemento), no tesis. SOBRE LOS INSUMOS: `correcionesa.txt` no contiene correcciones (reporte interno de otro entorno + fragmento del transcript de la terminal, pegado accidental); el DOCX no trae comentarios ni control de cambios y lo modificó por última vez el propio autor, pero difiere del generado por md2ieee_docx.py en exactamente 8 ediciones escritas sobre el texto — 4 correcciones reales, que SOBREVIVEN en el artículo actual, y 2 erratas introducidas al editar, que no se arrastraron. CINCO DECISIONES POSTERIORES A SU REVISIÓN que la tutora no ha visto y debe avalar: el modelo cede el primer plano a V(fallo,K) y pierde su figura; "trabajo futuro" pasa a "Fase 2 ya diseñada" (factorial 2×2, pese a que el entorno productivo no permite instalar otros operadores); se rebaja la novedad de F2; los datos pasan de "a petición" a depósito público; y cambia la revista de destino. NO CONTESTABLE con este material: si el artículo respeta el espíritu de sus recomendaciones de fondo, porque no hay ninguna en los archivos aportados.
**Report:** quality_reports/reviews/2026-08-17_consecuencia-con-documento-revisado-tutora.md

### 2026-08-18 — writer (paso de tiempo verbal, recomendación de la Prof. Mirella)
**Phase:** Execution (revisión de supervisión académica)
**Target:** paper/faraute/main.tex
**Score:** N/A (edición de redacción autorizada por el autor)
**Verdict:** Aplicado el criterio "pasado para lo que ocurrió; presente para lo que el artículo afirma, define o muestra" tras auditar las 298 oraciones del cuerpo (no solo las 9 de la primera pasada). 21 verbos en 14 oraciones: Grupo A, mezclas de tiempo dentro de una misma frase —"El clúster es productivo … no permitía … el riesgo que ello implica" → era/permitía/implicaba; "El manifiesto solicita … pero el pool se reducía" → solicitaba; "El RPO fue nulo … pero solo F1 pone a prueba" → puso; "No es un error de configuración (… eran correctos)" → No fue—. Grupo B, presente que describía trabajo ejecutado: respondió (×2), fue/era intra-nodo, toleraban, se reportaron (×2), fueron descriptivas, se concentró, se observó, Emergió, fue 4,65×, fue completa, confirmó, exhibió. DELIBERADAMENTE EN PRESENTE (defendible ante la tutora): el párrafo del mecanismo de §6.1 —pasarlo a pasado degradaría la contribución central de "así funciona CNPG" a "eso pasó aquel día"—, el resumen ES/EN, las referencias a tablas y figuras, el modelo y los invariantes, las propiedades del software ajeno, las relaciones lógicas y las decisiones editoriales del artículo. Verificado tras recompilar: **12 pp**, 0 overfull, 0 refs sin resolver, 0 DeviceRGB, cajas sueltas sin variación (13). Descartadas las sugerencias de marcar título y resumen como Work in Progress: el destino confirmado por el autor es la revista Faraute, donde esa modalidad no existe.
**Report:** quality_reports/reviews/2026-08-18_recomendaciones-tutora-mirella.md
